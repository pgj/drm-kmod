...
#ifdef __FreeBSD__
        ret = 0;
        gt->bsd_resource_id = 0x100;
        gt->bsd_resource = bsd_intel_pci_bus_alloc_mem(
            dev->bsddev, &gt->resource_id, len,
            &gt->resource.start, &gt->resource.end);
        if (gt->bsd_resource == NULL)
                ret = -ENOMEM;
#else
...
#ifdef __FreeBSD__
        if (gt->bsd_resource != NULL) {
                bsd_intel_pci_bus_release_mem(dev->bsddev, gt->resource_rid, gt->bsd_resource);
                gt->bsd_resource = NULL;
        }
#else
